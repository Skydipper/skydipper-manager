...
class TagsForm extends React.Component {
  ...
  loadDatasetTags() {
    ...
    this.graphService
      .getDatasetTags(this.props.dataset)
      .then(response => {
        const knowledgeGraphVoc = response.find(elem => elem.id === 'knowledge_graph');
        const datasetTags = knowledgeGraphVoc
          ? knowledgeGraphVoc.attributes.tags
          : knowledgeGraphVoc;
        this.setState(
          {
  ...
  loadSubGraph() {
    ...
    this.setState({
      graph: {
        edges: this.knowledgeGraph.edges.filter(elem =>
          inferredTags.find(tag => tag.id === elem.to)
        ),
        nodes: this.knowledgeGraph.nodes
          .filter(elem => inferredTags.find(tag => tag.id === elem.id))
          .map(elem => ({
            ...elem,
            color: selectedTags.find(tag => tag === elem.id) ? '#c32d7b' : '#F4F6F7'
          }))
      }
    });
  }
