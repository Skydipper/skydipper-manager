          ...
          <nav className={`header-menu-mobile-nav ${classNames}`}>
            ...
            <ul>
              {APP_HEADER_ITEMS.map(item => {
                ...
                if (typeof item.manager !== 'undefined' && item.manager !== isUserAdmin)
                  return null;
                    ...
                    {item.children && (
                      <ul>
                        {item.children.map(c => {
                          ...
                          // If admin user is defined and is not equal to the current token
                          if (typeof c.manager !== 'undefined' && c.manager !== isUserAdmin) {
                            return null;
                          }
                          ...
                          return (
                            <li key={c.label}>
                              ...
                              {!!c.href && <a href={c.href}>{c.label}</a>}

                              {c.id === 'logout' && (
                                <a onClick={this.logout} href={c.href}>
                                  {c.label}
                                </a>
                              )}
                            </li>
