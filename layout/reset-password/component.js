...
class ResetPassword extends PureComponent {
          ...
          .catch(err => {
            ...
            err.json().then(({ errors } = {}) => {
              (errors || []).forEach(_error =>
                toastr.error('Something went wrong', `${_error.status}:${_error.detail}`)
              );
            });
          });
      });
  ...
  render() {
    const { password, repeatPassword, saving } = this.state;

    return (
      <Layout title="Skydipper Sign-in/Register" description="Skydipper Sign-in/Register">
