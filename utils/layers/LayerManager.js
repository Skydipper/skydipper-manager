  ...
  addCartoLayer(layerSpec, verifyLayersOnly = false, callback) {
    ...
    fetch(`https://${layer.account}.carto.com/api/v1/map${params}`)
      ...
      .then(data => {
        ...
        const tileUrl = `${data.cdn_url.templates.https.url}/${layer.account}/api/v1/map/${
          data.layergroupid
        }/{z}/{x}/{y}.png`;
        ...
        // Add interactivity
        if (isInteractive) {
          const gridUrl = `https://${layer.account}.carto.com/api/v1/map/${
            data.layergroupid
          }/0/{z}/{x}/{y}.grid.json`;
          this.interactionLayers[layer.id] = L.utfGrid(gridUrl)
            .addTo(this.map)
            .setZIndex(995);
