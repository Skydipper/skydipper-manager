export default class GraphService {
  ...
  getAllTags() {
    return fetch(
      `${process.env.WRI_API_URL}/graph/query/list-concepts?application=${
  ...
  getInferredTags(tags) {
    return fetch(
      `${process.env.WRI_API_URL}/graph/query/concepts-inferred?concepts=${tags}&application=${
        process.env.APPLICATIONS
      }`,
      { headers: { 'Upgrade-Insecure-Requests': 1 } }
    )
  ...
  getDatasetTags(datasetId) {
    return fetch(
      `${process.env.WRI_API_URL}/dataset/${datasetId}/vocabulary?application=${
        process.env.APPLICATIONS
      }`,
      { headers: { 'Upgrade-Insecure-Requests': 1 } }
    )
  ...
  countDatasetView(datasetId, token) {
    ...
    return fetch(
      `${process.env.WRI_API_URL}/graph/dataset/${datasetId}/visited?application=${
        process.env.APPLICATIONS
      }`,
      {
        method: 'POST',
  ...
  getMostFavoritedDatasets() {
    return fetch(
      `${process.env.WRI_API_URL}/graph/query/most-liked-datasets?application=${
        process.env.APPLICATIONS
      }`,
      { headers: { 'Upgrade-Insecure-Requests': 1 } }
    )
