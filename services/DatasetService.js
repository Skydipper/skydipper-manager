...
export default class DatasetService {
  ...
  getSubscribableDatasets(includes = '') {
    return fetch(
      `${this.opts.apiURL}/dataset?application=${process.env.APPLICATIONS}&language=${
        this.opts.language
      }&includes=${includes}&subscribable=true&page[size]=999`,
  ...
  fetchData(includes = '', applications = [process.env.APPLICATIONS]) {
    const url = `${this.opts.apiURL}/dataset/${this.datasetId}?application=${applications.join(
      ','
    )}&language=${this.opts.language}&includes=${includes}&page[size]=999`;
    return fetch(url, {
      method: 'GET',
      headers: { 'Upgrade-Insecure-Requests': 1 }
    })
  ...
  fetchDataset(includes = '', applications = [process.env.APPLICATIONS]) {
    ...
    return fetch(url, { headers: { 'Upgrade-Insecure-Requests': 1 } })
      .then(response => {
        if (response.status >= 400) throw Error(response.statusText);
        return response.json();
      })
  ...
  getLayers() {
    return fetch(
      `${this.opts.apiURL}/dataset/${this.datasetId}/layer?app=${process.env.APPLICATIONS}`,
      {
        headers: { 'Upgrade-Insecure-Requests': 1 }
      }
    )
  ...
  getSimilarDatasets(datasetIds, withAncestors = true) {
    ...
    return fetch(
      `${this.opts.apiURL}/graph/query/${endpoint}/?dataset=${datasetIds}&published=true&env=${
        process.env.API_ENV
      }&application=${process.env.APPLICATIONS}&limit=6`,
      { headers: { 'Upgrade-Insecure-Requests': 1 } }
    )
      .then(response => {
        if (response.status >= 400) throw new Error(response.statusText);
  ...
  static getDatasets(
    ...
    return fetch(
      ...
      }/dataset/?ids=${datasetIDs}&language=${language}&includes=${includes}&application=${applications.join(
        ','
      )}&page[size]=999`,
      { headers: { 'Upgrade-Insecure-Requests': 1 } }
    )
  ...
  searchDatasetsByConcepts(topics, geographies, dataTypes) {
    let counter = 0;
    const topicsSt = topics
      ? topics.map((val, index) => `concepts[${counter}][${index}]=${val}`).join('&')
      : null;
    if ((topics || []).length) counter++;
    const geographiesSt = geographies
      ? `${geographies.map((val, index) => `concepts[${counter}][${index}]=${val}`).join('&')}`
      : null;
    if ((geographies || []).length) counter++;
    const dataTypesSt = dataTypes
      ? `${dataTypes.map((val, index) => `concepts[${counter}][${index}]=${val}`).join('&')}`
      : null;
    ...
    return fetch(
      `${this.opts.apiURL}/graph/query/search-datasets?${querySt}&published=true&env=${
        process.env.API_ENV
      }&application=${process.env.APPLICATIONS}&page[size]=999999`,
      { headers: { 'Upgrade-Insecure-Requests': 1 } }
    )
      .then(response => {
        if (response.status >= 400) throw new Error(response.statusText);
